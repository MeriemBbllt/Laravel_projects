$(document).ready(function() {
    ...
    if ($("#addToCartModal").length) {
        $(document).on('click', '.add-to-cart-modal', function (e) {
            $.get("/product_modal/" + $(this).data('id'), function( data ) {
                $("#addToCartModal").find('.modal-body').html(data);
                $("#addToCartModal").modal('show');
                $("input.number-modal").inputSpinner();
            });
        });
        $(document).on('click', '#addToCartModal .close', function() {
            $("#addToCartModal").modal('hide');
        });
    }

    $(document).on('click', '.add-to-cart-btn', function (e) {
        let cartType = $(this).data('cart-type');
        let productType = $(this).data('product-type');
        let productId = $(this).data('product-id');
        let method = $(this).data('method');
        let quantity = parseInt($('.cart_q' + productId).val());
        if (0 == productType) {
            let subproducts = [];
            $(".subproduct-quantity").each(function(elem) {
                let sp = $(this);
                if (sp.data('id')) {
                    subproducts.push({
                        id: sp.data('id'),
                        quantity: parseInt(sp.val()),
                    });
                }
            });
            cart.add(productId, quantity, cartType, 0, 0, 0, subproducts, productType);
        } else {
            let color = parseInt($('input[name="colors"]:checked').val());
            let cover = parseInt('modal' == method ? $('select[name="covers"]').val() : $('input[name="covers"]:checked').val());
            let slide = parseInt('modal' == method ? $('select[name="slide"]').val() : $('input[name="slide"]:checked').val());
            cart.add(productId, quantity, cartType, color, cover, slide);
        }
    });
    ...
    $(".cov_file").on("click", function() {
        var ids = [];
        $(".cov_file:checked").each(function(){
            ids.push($(this).val());
        });
        if (ids.length > 0) {
            get_covers(ids);
...
number_of_pg=element.number_of_pages;
name_fl=element.file;
                        ...
                        if (element.price_id > 0) {
                            $(".covers_files_container").append(
                                "<tr " +
                                    ...
                                    "</div>" +
                                    "</td>" +
                                    "<td><span style='display: block;width: 300px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;'>" +
                                    element.file +
                                    "</span></td>" +
                                    "<td class='max_num_" +
                                    element.id +
                    ...
                    json.data.covers.forEach(function(element) {
                        $(".covers_files").append(
                            "<tr>" +
                                ...
                                element.id + '" type="number" min="1" value="' +
                                element.quantity +
                                '"></td>' +
    ...
    $(".input-b1")
        .fileinput({
            ...
            browseLabel: "تصفح ملفاتك",
            autoReplace: false,
            allowedFileExtensions: ['pptx', 'docx', 'pdf', 'doc', 'ppt','PDF','PPT','DOCX','PPTX','DOC','png','jpg','jpeg','PNG','JPG','JPEG'],
                    ...
                    if (json["success"] == "1") {
                        ...
                        json.data.covers.forEach(element => {
                            ...
                            $(".covers_radios").append(
                                '<div class="pretty p-icon p-toggle p-smooth p-plain ">' +
                                    '  <input type="radio"  value="' +
                                    element.id +'" class="cover_id" max1 ="'+element.id+
                                    '" name="cover_id" />' +
                                    '  <div class="state ">' +
                                    '     <p class="icon">' +
                                    '         <img src="' +
    ...
    $(".next-branch").on("click", function(e) {
        ...
        $.ajax({
            ...
            success: function(json) {
                // Need to set timeout otherwise it wont update the total
                if(json["success"] == "1"){
                    console.log("getCoupanDiscount start");
                    getCoupanDiscount('code');
                    console.log("getCoupanDiscount end");
                    var x = document.getElementById("snackbar");
                    $("#snackbar").text(json["message"]);
                    $(".arrival_price").text(json.arrival_price+' ريال');

                    $(".totalAll").text(json.total+' ريال');
                    $(".tttt").text(json.total);
                    x.className = "show";
                    setTimeout(function() {
                        x.className = x.className.replace("show", "");
                    }, 3000);
            ...
            $.ajax({
                ...
                success: function(json) {
                    if (json.success == 1) {
                        ...
                        json.data.covers.forEach(element => {
                            var photo = "/uploads/cover_type/" + element.photo;
                            number_max_pg=element.max_end_to;
                            $(".covers_radios").append(
                                '<div class="pretty p-icon p-toggle p-smooth p-plain ">' +
                                    '  <input type="radio"  value="' +
                                    element.id +
                                    '" max2 ="'+element.max_end_to +
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(json) {
                                            if (json.success == 1) {
                                                ...
                                                var selectedCoverId = $(".cover_id:checked").val();
                                                var maxPages = $(".cover_id:checked").attr('max2');
                                            
                                                if ((selectedCoverId == 11 || selectedCoverId == 10 || selectedCoverId == 9 || selectedCoverId == 8 || selectedCoverId == 7|| selectedCoverId == 6 || selectedCoverId == 4) && (parseInt(number_of_pg) >= parseInt(maxPages))) {
                                                    var alertMessage = "سيتم تقسيم ملف '" + name_fl + "' نظرا لتجاوزه عدد الصفحات المحددة للتغليف وهي " + maxPages + " صفحة وسيتم اضافة تغليف اخر لتجنب تقسيم الملف يرجى التأكد من اختيار أوجه الطباعة الى وجهين";
...
$(".product_prop_price").on("click", function() {
    var cover = $(".cover:checked").val();
        var color = $(".color:checked").val();
        var side = $(".side:checked").val();

  var product_id =  $(".product_id").val();
    $.ajax({
        url: "/product_prop_price/"+product_id,
        type: "post",
        data: {
            _token: $("meta[name=csrf-token]").attr("content"),
            color: color,
...
function splite_file_html(file_id) {
    ...
    if (parseInt(to) > parseInt(max)) {
        ...
        return false;
    } else if (parseInt(to) > parseInt(max)) {
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if (json.success == "1") {
                ...
                if(json.wallet_amount > json.total){
                    $(".wallet_pay").show();
                }else{
                   $(".wallet_pay").hide();
                }
